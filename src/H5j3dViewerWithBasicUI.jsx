...
import { disableSpinnerRepeat, fixVolumeSize, parseH5jAttrs, surfaceAlignmentFactors, textFromFileOrURL } from './Utils';
...
function H5j3dViewerWithBasicUI() {
  ...
  const onDtScaleChange = (event) => {
    // Throttling does not seem to work very well for these numeric inputs,
    // so as an alternative, use a heuritic to disable repeat on the 
    // "spinner" (the little up and down arrow buttons).
    disableSpinnerRepeat(event, dtScale);

    setDtScale(event.target.valueAsNumber);
  }

  const onPeakChange = (event) => {
    // See the comment above about throttling.
    disableSpinnerRepeat(event, peak);
  ...
  const onDataGammaChange = (event) => {
    // See the comment above about throttling.
    disableSpinnerRepeat(event, dataGamma);
  ...
  const onFinalGammaChange = (event) => {
    // See the comment above about throttling.
    disableSpinnerRepeat(event, finalGamma);
